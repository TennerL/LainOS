...
key_event_t keyboard_read_key(void) {
    ...
    for (;;) {
        ...
        if (extended) {
            switch (code) {
                case 0x4B: return (key_event_t) { KEY_LEFT, 0 };
                case 0x4D: return (key_event_t) { KEY_RIGHT, 0};
                case 0x48: return (key_event_t) { KEY_UP, 0};
                case 0x50: return (key_event_t) { KEY_DOWN, 0};
                default: return none;
            }
        }
        ...
        switch (code) {
            case 0x01: return (key_event_t) { KEY_ESC, 0};
            case 0x1C: return (key_event_t) { KEY_ENTER, '\n' };
            case 0x0E: return (key_event_t) { KEY_BACKSPACE, '\b' };
            case 0x0F: return (key_event_t) { KEY_TAB, '\t' };
            default: break;
        }

        char ch = scancode_to_ascii(code);
        if(ch) {
            return (key_event_t){ KEY_CHAR, ch };
        }
    }
}
